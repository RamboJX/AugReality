...
// to hold the result
//
static Boolean convertFileRepresentation (char *fileName, short inStyle, short outStyle)
{
	if (fileName == NULL) {
		return (false);
	}
...
)
{
    ...
    //by rainbo, output the mesh's name
    fprintf(fp, "o %s\n", mdagPathNodeName.asChar());
...
MStatus ObjTranslator::exportAll( )
{
    ...
    // by bubble, now we get the lookuptable , so we know the total number of the objects
    // and print the numbers to the file
    fprintf(fp, "objectNum %d\n", objectNodeNamesArray.length());
